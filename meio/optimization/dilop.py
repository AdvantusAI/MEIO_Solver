...
from ..utils.caching import cached  # If available in your codebase

logger = logging.getLogger(__name__)

# Original class name from your codebase
    ...
    def calculate(network, default_service_level=0.95):
        ...
        for node_id, node in network.nodes.items():
            ...
            service_level = 0.98 if (node.node_type == "store" and 'S' in node_id and 
                                     node_id[1:].isdigit() and int(node_id[1:]) <= 5) else default_service_level
...
# Alias for backward compatibility with our enhanced code
DilopSafetyStock = DiloptOpSafetyStock